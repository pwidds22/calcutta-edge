...
export async function middleware(request: NextRequest) {
  ...
  if (path.startsWith('/api/webhooks') || path.startsWith('/api/test-') || path.startsWith('/api/espn') || path.startsWith('/api/golf') || path.startsWith('/api/soccer') || path.startsWith('/api/worldcup') || path.startsWith('/ingest')) {
    return supabaseResponse
  }
