...
// ONLY the 7 that got rate-limited. These did NOT receive the email.
// Already sent successfully (DO NOT re-send):
//   Email A: dannyfein, tjh2310, jeff.lorenz, griffinkosonocky, ndweiss23,
//            tals400, recoils_fodders_0q, rafael.rojoz, chaselennartson, jose.rojozam
//   Email B: davies.max.21, tdemps58, rayfeliu, tony55serrano, joshuaboutt
...
<div style="${styles.wrapper}">
  <div style="${styles.container}">
    ...
    <div style="${styles.body}">
      <p style="${styles.p}">Hey,</p>
      <p style="${styles.p}">Quick heads up from the Survive the Chop team — we also built a March Madness Calcutta platform.</p>
      <p style="${styles.p}">If your group runs a Calcutta auction, you can now do the whole thing from your phones <strong style="color: #10b981;">for free</strong>:</p>
      <div style="${styles.featureBlock}">
        <p style="${styles.featureTitle}">🎯 Live Auction Hosting</p>
        <p style="${styles.featureDesc}">Create a room with a 6-digit code. Real-time bidding with countdown timers across all devices.</p>
      </div>
      <div style="${styles.featureBlock}">
        <p style="${styles.featureTitle}">📊 Automatic Payouts</p>
      ...
      <hr style="${styles.divider}" />
      <p style="${styles.p}"><strong style="color: #fff;">Games start today.</strong> If this is relevant to your group:</p>
      <div style="margin: 24px 0;">
        <a href="https://calcuttaedge.com" style="${styles.cta}">Check It Out →</a>
      </div>
      <p style="${styles.p}">— Patrick<br /><span style="color: #6b7280;">Survive the Chop / Calcutta Edge</span></p>
    </div>
    <div style="${styles.footer}">
      <p style="${styles.footerText}">
        <a href="https://calcuttaedge.com" style="color: #10b981; text-decoration: none;">calcuttaedge.com</a><br />
        You're receiving this because you're a Survive the Chop subscriber.
      </p>
    </div>
  </div>
</div>`;

const retryList: { email: string; subject: string; html: string }[] = [
  // Email A failures (4)
  { email: 'ira.gluck@gmail.com', subject: 'Your Calcutta tool got a massive upgrade', html: emailAHtml },
  { email: 'ryansg23@gmail.com', subject: 'Your Calcutta tool got a massive upgrade', html: emailAHtml },
  { email: 'nightninjarag@gmail.com', subject: 'Your Calcutta tool got a massive upgrade', html: emailAHtml },
  { email: 'kraftm12469@gmail.com', subject: 'Your Calcutta tool got a massive upgrade', html: emailAHtml },
  // Email B failures (3)
  { email: 'blitzology@gmail.com', subject: 'March Madness Calcutta — free auction tool', html: emailBHtml },
  { email: 'craig.williams184@hotmail.com', subject: 'March Madness Calcutta — free auction tool', html: emailBHtml },
  { email: 'stephengrivers1@gmail.com', subject: 'March Madness Calcutta — free auction tool', html: emailBHtml },
];

async function retry() {
  console.log(`Retrying ${retryList.length} failed emails (500ms delay between each)...\n`);
  ...
  for (const item of retryList) {
    ...
    try {
      ...
      if (error) {
        console.error(`  ✗ ${item.email}: ${error.message}`);
      } else {
        console.log(`  ✓ ${item.email} (${data?.id})`);
      }
    } catch (err) {
      console.error(`  ✗ ${item.email}:`, err);
    }
  }
